...
4. ใช้หมายเหตุเป็นรายละเอียดก่อน ถ้ายังไม่ทราบยอด/วันเวลา/รายละเอียด ให้ถามเติม ตอบตรง ๆ ได้เมื่อมีรายการค้างเพียงรายการเดียว หากมีหลายรายการให้เลือกด้วยเลขเรียงง่าย ๆ เช่น `1 ค่าอาหาร` หรือ `ยกเลิก 2`
...
คำสั่งหลัก: `ส่งสลิป`, `เพิ่มรายการ`, `ดูรายจ่าย`, `รายการค้าง`, `สรุปวันนี้`, `สรุปเดือนนี้`, `รายการล่าสุด`, `แก้รายการล่าสุด`, `ลบรายการล่าสุด`, `ส่งออกข้อมูล`, `ล้างประวัติ`, `ช่วยเหลือ`
...
ระบบจำรายละเอียดและหมวดจากร้านเดิมในประวัติของผู้ใช้คนนั้นเพื่อเสนอค่าเริ่มต้น โดยไม่เรียนรู้ข้ามผู้ใช้และยังให้ตรวจยืนยันทุกครั้ง เจ้าของตรวจคิว งานล้มเหลว cleanup และโควตา LINE ด้วย `สถานะระบบ` ได้

Rich Menu มี 4 ปุ่มขนาดใหญ่: ส่งสลิป, เพิ่มรายการ, ดูรายจ่าย และเมนูเพิ่มเติม ปุ่มสุดท้ายเปิดการ์ดในแชตสำหรับรายการค้าง สรุปวันนี้ สรุปเดือนนี้ จัดการรายการ และข้อมูลของฉัน ภาพ Rich Menu สร้างจากโทนเดียวกับโลโก้/พื้นหลังของ LINE OA และเก็บไว้ใน `example-slip/` เฉพาะเครื่องผู้พัฒนา
...
ระบบเริ่มจากเจ้าของผ่าน owner pairing หรือ User ID ตัวแรกใน `LINE_ALLOWED_USER_IDS` ข้อมูลทุกคนแยกด้วย LINE user ID

ทางเลือกในการแชร์:

1. เจ้าของพิมพ์ `เชิญเพื่อน` เพื่อรับรหัสใช้ครั้งเดียว อายุ 24 ชั่วโมง
2. เพื่อนแอด OA แล้วพิมพ์ `เข้าร่วม <รหัส>`
3. เจ้าของพิมพ์ `ผู้ใช้งาน` เพื่อดูสมาชิก และ `ถอนสิทธิ์ 1` เพื่อยกเลิกสิทธิ์ตามเลขลำดับ

Webhook จำกัดผู้ใช้แต่ละคนไว้ 30 events ต่อนาทีเพื่อกัน burst และตรวจสิทธิ์ซ้ำก่อนทำงานจากคิว ผู้ใช้ดู ส่งออก แก้ หรือลบได้เฉพาะข้อมูลของตนเอง
